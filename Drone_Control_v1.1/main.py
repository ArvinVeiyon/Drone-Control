...
import sys, os, logging, paramiko, re, time
from PyQt5.QtWidgets import QApplication, QMainWindow, QDockWidget, QWidget, QVBoxLayout, QHBoxLayout, QPushButton, QLabel, QStackedWidget, QTextEdit, QMessageBox, QLineEdit, QSizePolicy, QCheckBox, QListWidget, QTabWidget, QToolButton, QStyle, QDialog, QDialogButtonBox, QPlainTextEdit, QFormLayout
...
class DroneControlApp(QMainWindow):
    def __init__(self):
        ...
        self.connection_check_interval = int(self.ssh_executor.ssh_config.get("connection_check_interval", 30000))  # In milliseconds
        self.camera_swapped = False
        self.companion_version_label = QLabel("Companion Version: N/A")  # Ensure it starts as "N/A"
        self.companion_version_label.setStyleSheet("font-size: 12pt; color: #FFFFFF;")
        self.is_rebooting_or_shutting_down = False  # Flag to track reboot/shutdown operations
        ...
        top_dock_layout.addWidget(self.refresh_btn, alignment=Qt.AlignRight)
        ...
        self.refresh_connection_status()  # Initial connection check
    ...
    def create_left_dock(self):
        ...
        style_menu_button(btnHome)
        btnSettings = QPushButton("Settings")
        style_menu_button(btnSettings)
        btnCompanionSSH = QPushButton("Companion SSH")
        style_menu_button(btnCompanionSSH)
        btnRelaySSH = QPushButton("Relay SSH")
        style_menu_button(btnRelaySSH)
        btnQGC = QPushButton("Launch QGC App")
        style_menu_button(btnQGC)
        btnExit = QPushButton("Exit")
        style_menu_button(btnExit)
    ...
    def create_home_page(self):
        ...
        btnRecordFront.setStyleSheet(f"""
            QPushButton {{
        ...
        btnRecordBottom.setStyleSheet(f"""
            QPushButton {{
                background-color: #10893E;
                color: white;
                border: 1px solid #555555;
    ...
    def toggle_camera_swap(self, new_state):
        ...
        self.show_success_message(f"Camera mapping is now: {state_str}")  # Direct GUI update
    ...
    def record_front(self):
        ...
        if self.ssh_executor.execute_command(command, f"Recording Front camera ({device}) for {dur} seconds.", "Failed to record front camera. Check SSH credentials or remote command."):
            time.sleep(int(dur) + 2)  # Wait for recording and file transfer
            success, transferred_path = self.ssh_executor.transfer_file(remote_path, local_path)
            if success:
    ...
    def create_conn_page(self):
        ...
        col1_layout.setSpacing(int(5 * SCALE))
        ...
        col2_layout.setSpacing(int(5 * SCALE))
        ...
        self.relay_password_entry = QLineEdit()
        self.relay_password_entry.setEchoMode(QLineEdit.Password)
        ...
        col3_layout.setSpacing(int(5 * SCALE))
        ...
        self.interval_entry = QLineEdit(str(self.connection_check_interval // 1000))  # Display in seconds for user input
        col3_layout.addWidget(self.interval_entry)
        self.timeout_entry = QLineEdit(str(self.ssh_executor.timeout))  # New field for timeout in seconds
        col3_layout.addWidget(QLabel("Timeout (seconds):"))
        col3_layout.addWidget(self.timeout_entry)
        self.max_attempts_entry = QLineEdit(str(self.ssh_executor.max_attempts))  # New field for max attempts
...
- Updated to version 1.1: Improved handling of companion computer reboots to suppress unnecessary timeout errors and enhance log clarity.<br>
- Aligned connection check timeout and retries for periodic and manual checks with settings page configuration.<br>
- Fixed reboot timeout issues to respect the configured timeout (e.g., 300 seconds).<br><br>

<b>Settings Details:</b><br>
- <b>Companion SSH Configuration:</b><br>
  - Primary IP: Default 10.5.6.100 (relay tunnel)<br>
  - Primary Port: Default 2222<br>
  - Secondary IP: Optional failover IP<br>
  - Secondary Port: Default 22<br>
  - Username: Default 'roz'<br>
  - Password: Stored in system keyring<br>
- <b>Relay SSH Configuration:</b><br>
  - Relay IP: Default 10.5.6.100<br>
  - Relay SSH Port: Default 22<br>
  - Username: Default 'vind-admin'<br>
  - Password: Stored in system keyring<br>
- <b>Periodic Connection Check:</b><br>
  - Enable: Toggle periodic checks (default: enabled)<br>
  - Interval: Seconds between checks (default: 30s)<br>
  - Timeout: Seconds for each connection attempt (default: 5s)<br>
  - Max Attempts: Number of retry attempts (default: 3)<br>
- <b>Camera Settings:</b><br>
  - Device: e.g., /dev/video0 or /dev/video2 (remote Linux)<br>
  - Resolution: e.g., 1920x1080<br>
  - FPS: e.g., 60<br>
  - Format: e.g., MJPG<br>
  - Swap Cameras: Toggles front (/dev/video2) and bottom (/dev/video0) mapping<br>
""")
    ...
    def refresh_connection_status(self):
        ...
        # Use settings from SSHExecutor for timeout and max attempts
        original_timeout = self.ssh_executor.timeout
        original_max_attempts = self.ssh_executor.max_attempts
        
        # Apply settings from the connection check configuration (if updated in settings)
        try:
            self.ssh_executor.timeout = int(self.ssh_executor.timeout)  # Ensure timeout is an integer
            self.ssh_executor.max_attempts = int(self.ssh_executor.max_attempts)  # Ensure max_attempts is an integer
        except (ValueError, AttributeError):
            logger.warning("Invalid timeout or max attempts in settings, using defaults.")
            self.ssh_executor.timeout = 5  # Default timeout
            self.ssh_executor.max_attempts = 3  # Default max attempts
        ...
        if reachable_ip:
            ...
            self.ssh_executor.restart_relay_ssh_tunnel()  # Attempt to restart tunnel if connection fails

        # Reset timeout and max_attempts to defaults or original values
    ...
    def periodic_connection_check(self):
        ...
        # Schedule the next periodic check using the interval from settings (in milliseconds)
    ...
    def update_companion_version(self):
        ...
        try:
            ...
            version = "N/A"  # Default to "N/A"
            if content:
                match = re.search(r"[\s:](\d+\.\d+)", content)
                version = match.group(1) if match else "N/A"
            self.companion_version_label.setText(f"Companion Version: {version}")  # Direct GUI update
        except paramiko.AuthenticationException as e:
            logger.error("SSH Authentication failed for version retrieval: %s", e)
            self.companion_version_label.setText("Companion Version: N/A")  # Direct GUI update
    ...
    def apply_connection_settings(self):
        ...
        try:
            ...
            if interval_sec <= 0:
                ...
                return
            self.connection_check_interval = interval_sec * 1000  # Convert to milliseconds
    ...
    def update_cam_params_config(self, device, resolution, fps, cam_format):
        config_path = "/etc/vision_streaming.conf"  # Remote Linux path
    ...
    def reboot_companion_and_restart_tunnel(self):
        ...
        # Use the configured timeout and max attempts for the reboot command
        original_max_attempts = self.ssh_executor.max_attempts
        original_timeout = self.ssh_executor.timeout
        
        # Apply settings from the connection check configuration for this operation
        try:
            self.ssh_executor.max_attempts = 1  # Only attempt once for reboot, as retries will fail
            self.ssh_executor.timeout = int(self.ssh_executor.timeout)  # Use configured timeout
        except (ValueError, AttributeError):
            logger.warning("Invalid timeout or max attempts in settings for reboot, using defaults.")
            self.ssh_executor.timeout = 300  # Default to 300 seconds if invalid
        ...
        if self.ssh_executor.execute_command_all("sudo reboot", "Companion computers are rebooting.", "Failed to initiate reboot of companion computers."):
            ...
            # Wait longer to ensure the system has time to reboot (increased from 60 to 90 seconds)
            time.sleep(90)  # Wait 90 seconds to allow systems to reboot fully
            self.refresh_connection_status()
        else:
            self.show_error_message("Failed to initiate reboot of companion computers.")
            self.ssh_executor.restart_relay_ssh_tunnel()  # Restart tunnel only if reboot initiation fails

        # Restore original max_attempts and timeout
        self.ssh_executor.max_attempts = original_max_attempts
        self.ssh_executor.timeout = original_timeout
    ...
    def shutdown_companion_and_restart_tunnel(self):
        ...
        # Use the configured timeout and max attempts for the shutdown command
        original_max_attempts = self.ssh_executor.max_attempts
        original_timeout = self.ssh_executor.timeout
        
        # Apply settings from the connection check configuration for this operation
        try:
            self.ssh_executor.max_attempts = 1  # Only attempt once for shutdown, as retries will fail
            self.ssh_executor.timeout = int(self.ssh_executor.timeout)  # Use configured timeout
        except (ValueError, AttributeError):
        ...
        if self.ssh_executor.execute_command_all("sudo shutdown now", "Companion computers are shutting down.", "Failed to shut down companion computers."):
            ...
            time.sleep(120)  # Wait 120 seconds to allow systems to shut down fully
            self.refresh_connection_status()
        else:
            self.show_error_message("Failed to shut down companion computers.")
            self.ssh_executor.restart_relay_ssh_tunnel()  # Restart tunnel only if shutdown initiation fails
    ...
    def reboot_relay(self):
        ...
        if self.ssh_executor.execute_relay_command("sudo reboot", "Relay station is rebooting.", "Failed to reboot relay station."):
            ...
            time.sleep(90)  # Wait 90 seconds to allow relay to reboot fully
            self.refresh_connection_status()
        else:
    ...
    def shutdown_relay(self):
        ...
        if self.ssh_executor.execute_relay_command("sudo shutdown now", "Relay station is shutting down.", "Failed to shut down relay station."):
            ...
            time.sleep(120)  # Wait 120 seconds to allow relay to shut down fully
            self.refresh_connection_status()
        else:
