...
import os, json, socket, time, keyring, logging, sys
...
class SSHExecutor:
    ...
    def execute_command(self, command, success_msg="Command executed successfully.", error_msg="Failed to execute command.", max_attempts=None):
        """
    ...
    def execute_command_all(self, command, success_msg="Command executed successfully on all systems.", error_msg="Failed to execute command on all systems."):
    ...
    # Relay commands
    # -------------------------------------------------------------------------
    def execute_relay_command(self, command, success_msg="Relay command executed successfully.", error_msg="Failed to execute relay command.", max_attempts=None):
    ...
    def get_wifi_module_temperature(self):
        """
        Prefer WFB's own CLI reading; fall back to rtl88x2eu proc/sysfs.
        For /proc/net/rtl88x2eu/.../thermal_state, parse per-RF-path lines and
        return the MAX temperature (conservative). Returns float °C or None.
        """
        import re
        ...
        try:
            ...
            # 1) Try wfb-cli first (same as what you run manually)
            wfb_cmd = r"""sh -lc 'TO=2; (timeout ${TO}s /usr/local/bin/wfb-cli drone || timeout ${TO}s wfb-cli drone) 2>/dev/null || true'"""
            stdin, stdout, stderr = ssh.exec_command(wfb_cmd)
            ...
            # 2) Fallback: parse /proc thermal_state (multi-line)
            proc_path = f"/proc/net/rtl88x2eu/{iface}/thermal_state"
            cmd = f"sh -lc 'timeout 2s cat {proc_path} 2>/dev/null || true'"
            stdin, stdout, stderr = ssh.exec_command(cmd)
            stdout.channel.settimeout(self.timeout)
            proc_out = stdout.read().decode(errors="ignore")

            # Extract all "... temperature: <num>" values
            temps = []
            for mm in re.finditer(r'temperature:\s*(-?\d+(?:\.\d+)?)', proc_out):
                try:
                    ...
                    pass
            if temps:
                # Conservative: report the hottest RF path
                return float(f"{max(temps):.1f}")

            # 3) Last fallback: sysfs hwmon temp1_input (degC or millidegC)
            sysfs_cmd = r"""sh -lc '
TO=2
...
'"""
            stdin, stdout, stderr = ssh.exec_command(sysfs_cmd)
            stdout.channel.settimeout(self.timeout)
            val_s = stdout.read().decode(errors="ignore").strip()
            if val_s:
                try:
                    v = float(val_s)
